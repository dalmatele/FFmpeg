/**
 * How to run it
 * after building, you can run this command: ./hls_enc big_buck_bunny.webm index.m3u8 
 */
...
//            av_opt_set(output_codec_context->priv_data, "hls_segment_type", "fmp4", 0);
    ...
    while (1) {
        ...
        if (in_packet.stream_index == video_stream_index) {
            ...
            while (ret >= 0) {
                ...
                if (ret >= 0) {
                    ...
                    while (ret >= 0) {
                        ...
                        ret = write_chunk(output_format_context, out_packet);
                        if (ret < 0) {
            ...
            ret = write_chunk(output_format_context, &in_packet);
            if (ret < 0) {
                logging("Error muxing packet\n");
                break;
            }
...
static int write_playlist() {
    ...
    char *playlist_name = "index.m3u8";
    int ret;
    ret = avio_open(&output_io_context, playlist_name, AVIO_FLAG_WRITE);
    if (ret < 0) {
        logging("Can not create playlist file: %s\n", av_err2str(ret));
        return ret;
    }
    ff_hls_write_init_file(output_io_context, playlist_name, 1, 10, 0);
    ff_hls_write_playlist_header(output_io_context, 6, 0, 5, 0, PLAYLIST_TYPE_EVENT, 0);
    avio_printf(output_io_context, "#EXT-X-DISCONTINUITY\n");
    ff_hls_write_end_list(output_io_context);
    avio_flush(output_io_context);
    return ret;
}
